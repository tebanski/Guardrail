...
# display readings on M5STACK TFT screen
DISPLAY_READINGS = const(True)

# log readings to file on SD card
STORE_READINGS = const(False)

# toggle to use cooked orientation readings from 9DoF sensor:
ORIENT_COOKED = const(False)
...
DATA_PATH = const("/sd/__DATA__")

# format to use for packing data for storage and transmission
PACK_FORMAT = const("QLLBBdddddddddddd")

# maximum number of samples per data file
MAX_ENTRIES_PER_FILE = const(600 * (1000 / SAMPLE_INTERVAL))
...
ACCELERATION_BURST_PEAK = const(1.5)
...
a_i2c = I2C(scl=Pin(22), sda=Pin(21))
mpu6500 = MPU6500(a_i2c, gyro_sf=SF_DEG_S)
sensor = MPU9250(a_i2c, mpu6500=mpu6650)
acc = Fusion()

# attach to GPS module
g_i2c = I2C(scl=Pin(26), sda=Pin(25))
receiver = GNSSL76L(g_i2c)
gps = MicroGPS()
...
def display_header():
  tft.font(font)
  tft.text(60, 20, "X-ACCEL:")
  tft.text(60, 40, "Y-ACCEL:")
  tft.text(60, 60, "Z-ACCEL:")
  if ORIENT_COOKED:
    tft.text(60, 80, "HEADING:")
    tft.text(88, 100, "PITCH:")
    tft.text(102, 120, "ROLL:")
  else:
    tft.text(88, 80, "X-ANGV:")
    tft.text(88, 100, "Y-ANGV:")
    tft.text(88, 120, "Z-ANGV:")
  tft.text(102, 140, "LATI:")
  tft.text(102, 160, "LONG:")
  tft.text(88, 180, "SPEED:")
...
def _collect_9dof(sensor):
  ...
  return dict(a=acceleration, g=gyro, m=magnetic)
...
def display_readings():
  ...
  tft.text(180, 20, "{:3.4f}\r".format(data_packet[4]))
  tft.text(180, 40, "{:3.4f}\r".format(data_packet[5]))
  tft.text(180, 80, "{:3.4f}\f".format(data_packet[6]))
  tft.text(180, 80, "{:3.4f}\r".format(data_packet[7]))
  tft.text(180, 100, "{:3.4f}\r".format(data_packet[8]))
  tft.text(180, 120, "{:3.4f}\r".format(data_packet[9]))
  tft.text(180, 140, "{:3.4f}\r".format(data_packet[13]))
  tft.text(180, 160, "{:3.4f}\r".format(data_packet[14]))
  tft.text(180, 180, "{:3.4f}\r".format(data_packet[15]))
